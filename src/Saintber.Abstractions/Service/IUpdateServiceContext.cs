...
namespace Saintber.Abstractions.Service
{
    ...
    public interface IUpdateServiceContext<TDataModel, TUpdateDataModel, TDto, TUpdateRequest>
    {
        ...
        /// </summary>
        Func<TUpdateRequest, CancellationToken, Task<TUpdateDataModel>> OnBeforeUpdateAsync { get; set; }

        /// <summary>
        /// 在更新後執行的處理程序，可用於進一步處理已更新的數據，例如觸發事件或更新快取。
        /// </summary>
        Func<TDataModel, CancellationToken, Task<TDto>> OnAfterUpdateAsync { get; set; }
    }
}
